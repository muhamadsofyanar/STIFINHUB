...
try{
  ...
  r=await fetch(`${base}/messages`,{method:'POST',headers:{cookie,'content-type':'application/x-www-form-urlencoded'},body:new URLSearchParams({csrf,leadId:String(leadId),message:'Halo {nama}',action:'send'})});assert(r.ok&&(await r.text()).includes('berhasil diteruskan'),'Pesan StarSender gagal');assert(starSenderPayload?.to==='6281234567890','Payload StarSender salah');
